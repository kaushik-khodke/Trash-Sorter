...
class ThreadSafeStateMachine:
    ...
    def update_analysis(self, analysis):
        with self.lock:
            ...
            # --- STATE 2: THINKING (2.5s observation) ---
                ...
                if elapsed >= config.THINKING_DURATION:
                    if self.sample_count > 0 and self.accumulated_scores:
                        ...
                        logging.info(f"DECISION LOCKED -> Transmitting Single-Letter Code [{self.latched_letter}] ({self.latched_display_name}) to robotic_hand.ino.")
                        logging.info("ENTERING ARM OPERATING STATE: Model guessing PAUSED until 'Done' received.")
                        
                        # Transmit single-letter code EXACTLY ONCE to robotic_hand.ino
...
def ai_worker_thread(classifier, state_machine, frame_queue, stop_event):
    """Background AI worker. Pauses processing when arm is operating."""
    while not stop_event.is_set():
        try:
            roi_crop = frame_queue.get(timeout=0.05)
            if roi_crop is not None:
                # Only analyze if not in ARM_OPERATING lock state
...
def draw_hud(frame, snapshot, iiot, fps):
    """Renders live video feed overlay."""
    h, w, _ = frame.shape

    # Central Target ROI Box
    roi_size = min(w, h) // 2
    rx1 = (w - roi_size) // 2
    ry1 = (h - roi_size) // 2
    rx2 = rx1 + roi_size
    ...
    cv2.putText(frame, "IIoT ROBOTIC ARM TRASH SORTER", (20, 35),
    ...
    if state == ThreadSafeStateMachine.STATE_WAITING:
        ...
        cv2.putText(frame, f"STATUS: ANALYZING WASTE ITEM... [{pct}%]", (30, h - 50),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 200, 255), 2, cv2.LINE_AA)
        
        bar_w = w - 60
        cv2.rectangle(frame, (30, h - 30), (30 + bar_w, h - 15), (40, 40, 40), -1)
        fill = int(bar_w * progress)
        cv2.rectangle(frame, (30, h - 30), (30 + fill, h - 15), (0, 200, 255), -1)

    elif state == ThreadSafeStateMachine.STATE_ARM_OPERATING:
        cat_color = config.CATEGORIES[snapshot["category"]]["color"] if snapshot["category"] else (0, 165, 255)
        
        # Giant Code Badge
        cv2.putText(frame, f"CODE: {letter}", (30, h - 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 1.8, cat_color, 4, cv2.LINE_AA)

        cv2.putText(frame, f"{display_name.upper()} - ROBOTIC ARM BUSY", (240, h - 45),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 200, 255), 2, cv2.LINE_AA)
        
        cv2.putText(frame, "MODEL PAUSED -> WAITING FOR ARDUINO 'DONE' ACKNOWLEDGEMENT | [R] Reload", (240, h - 20),
...
def main():
    ...
    # Initialize Serial IIoT communicator matching robotic_hand.ino (19200 baud)
    ...
    logging.info("Initializing High-Performance Vision Engine...")
    ...
    print("\n=======================================================")
    print("  IIOT ROBOTIC ARM TRASH SORTER (robotic_hand.ino)")
    print("=======================================================")
    print("Exact Code Mappings (robotic_hand.ino):")
    print("  [P] -> Plastic Waste   (Line 66)")
    print("  [A] -> Paper Waste     (Line 82)")
    print("  [C] -> Cardboard Waste (Line 98)")
    print("  [G] -> Glass Waste     (Line 114)")
    print("  [M] -> Metal Waste     (Line 130)")
    print("-------------------------------------------------------")
    print("State Protection:")
    print("  - Model PAUSES when Arm is Operating (No double commands)")
    print("  - Resumes ONLY after Arduino emits 'Done' signal")
    print("Controls:")
    print("  - Press 'R' to Reload / Reset system state")
    ...
    try:
        while True:
            ...
            cv2.imshow("IIoT Waste Sorter Live View", display_frame)

            key = cv2.waitKey(1) & 0xFF
            if key == ord('q') or key == 27:
                break
            elif key == ord('r') or key == ord('R'):
                # Manual Reload / Reset Button
